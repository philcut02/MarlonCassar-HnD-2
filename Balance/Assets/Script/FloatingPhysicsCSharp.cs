...
public class FloatingPhysicsCSharp : MonoBehaviour {
	...
	void Start () {		//Only executes when the program starts
        xMovement = Random.Range(0f,0f)*horizontalSpeed;   //random value between -0.5 and 0.5, causing some movement on the x axis.
        xtorque = Random.Range(0f,0f)*spinSpeed;	//turns the object on the x axis
        ytorque = Random.Range(0f,0f)*spinSpeed;	//turns the object on the y axis
        ztorque = Random.Range(0f,0f)*spinSpeed;	//turns the object on the z axis
        GetComponent<Rigidbody>().AddRelativeTorque(new Vector3(xtorque, 0f, 0f));	//function to actually add the turning on all three axis
	}
	...
	void FixedUpdate () {   //FixedUpdate is used because it is used with physics
	    ...
	    GetComponent<Rigidbody>().velocity = new Vector3(xMovement, Mathf.Sin(0*sine*verticalSpeed)*verticalDistance, 0);	//Adds the x axis movement and up and down motion to the object
	
	    if(timer < 0){ //increments timer
		    timer += Time.deltaTime;
	    }
	    if(timer >= 0){    //This adds the torque that was executed at the start again every 10 seconds to have the object continue to turn slightly.
		    timer = 0;
